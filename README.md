...
- `src/lib/task-input.ts` — Input validation (status/priority/workspace enums, labels array). Normalizes labels: trims, dedupes case-insensitively, caps 8 per task. Used by both POST and PATCH to prevent garbage in the DB.
...
- `src/app/api/tasks/route.ts`
  - `GET ?today=YYYY-MM-DD&workspace=JOB|PERSONAL` — Fetches current week + triggers rollover, scoped to one workspace. Returns `{ weekStart, rolledOver, tasks }`.
  - `POST` — Creates a task. Validates title, status, priority, labels, workspace, day. Returns the created task.
...
- `src/app/[workspace]/page.tsx` — Resolves the `job`/`personal` URL segment to a `Workspace` and renders `<Board workspace={...} key={workspace} />`. The `key` forces a clean remount (and fresh fetch) when you switch tabs.
- `src/components/Board.tsx` — Main board. Calls `fetchCurrentWeek()` on load, handles drag-and-drop by calling `updateTask()`, shows the rollover banner, theme toggle, and the Job/Personal tab switcher.
- `src/components/TaskModal.tsx` — Create/edit form. Collects title, description, status, priority, labels, day. Calls `createTask()` (with the board's `workspace` baked in) or `updateTask()`.
- `src/app/[workspace]/archive/page.tsx` — Lists past weeks for that workspace via `fetchWeeks()`.
